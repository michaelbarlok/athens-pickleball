...
// ============================================================
// Types — exported so the server `page.tsx` shell can pass them
// in as props without redefining them.
// ============================================================
...
interface Props {
  groupId: string;
  initialTab: Tab;
  initialGroup: ShootoutGroup;
  initialPreferences: GroupPreferences | null;
  initialMembers: MemberRow[];
  initialAllPlayers: Profile[];
...
export function AdminGroupClient({
  groupId,
  initialTab,
  initialGroup,
  initialPreferences,
  ...
  // Use a stable local alias so the existing handler bodies that
  // referenced `id` from useParams still compile unchanged.
  const id = groupId;
  const { supabase } = useSupabase();
  const router = useRouter();

  // Server-fed state. The useEffects below resync these when the
  // server re-renders (router.refresh after mutations + realtime
  // events). Local mutations still set state optimistically so the
  // UI feels instant; the server's refreshed props win on the next
  // refresh, which keeps the client in sync with reality.
  const [group, setGroup] = useState<ShootoutGroup>(initialGroup);
  const [preferences, setPreferences] = useState<GroupPreferences | null>(
    initialPreferences
  );
  const [members, setMembers] = useState<MemberRow[]>(initialMembers);
  const [allPlayers, setAllPlayers] = useState<Profile[]>(initialAllPlayers);
  ...
  // Realtime: re-fetch members when group_memberships change
  // (e.g. step updates from a shootout). router.refresh re-runs the
  // page server component, the new props flow down, and the
  // sync-from-props useEffects above pick up the fresh data.
  useEffect(() => {

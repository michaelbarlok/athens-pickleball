...
/**
 * Resize + re-encode any uploaded image to a 512px-max JPEG. HEIC
 * from iOS photos is decoded via heic2any first. The user never has
 * to crop or resize — we always produce a small uniform asset for
 * the public club page. Same pattern as TournamentLogoUpload.
 */
...
export function ClubLogoUpload({ clubId, currentUrl, onUploaded }: Props) {
  ...
  const handle = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file) return;
      if (!clubId) {
        setError("Save the club first, then upload a logo.");
        return;
      }
      if (file.size > MAX_RAW_BYTES) {
        setError(
          `That image is ${(file.size / 1024 / 1024).toFixed(1)}MB — too large. Try one under 10MB.`
        );
        return;
      }
      setError(null);
    ...
    [supabase, clubId, currentUrl, onUploaded]
  );
  ...
  return (
    <div className="flex items-start gap-4">
      ...
      >
        {preview ? (
          <img src={preview} alt="Club logo preview" className="h-full w-full object-contain p-1" />
        ) : (
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <p className="text-sm font-medium text-dark-100">Club logo (optional)</p>
        <p className="text-xs text-surface-muted">
          {clubId
            ? "Any image works — we'll resize it to 512px and re-encode for you."
            : "Save the club first; you can add a logo from the manage page after."}
        </p>
